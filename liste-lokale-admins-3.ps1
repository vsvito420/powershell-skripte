...
# Filtere Ergebnisse für die Tabelle der PCs mit Administratoren
$AdminsFound = $Results | Where-Object { $_.Status -eq "Erfolg" -and $_.Admins -ne "N/A" -and $_.Admins -ne "" }

# Erstelle die erste HTML-Tabelle (PCs mit Administratoren)
$HtmlTableAdmins = $AdminsFound | ConvertTo-Html -Property ComputerName, Admins, Status -Fragment | ForEach-Object {
    $_ -replace '<td>Fehler:', '<td class="error">Fehler:' -replace '<td>Erfolg</td>', '<td class="success">Erfolg</td>'
}

# Erstelle die zweite HTML-Tabelle (Gemischte Liste aller PCs)
$HtmlTableAll = $Results | ConvertTo-Html -Property ComputerName, Admins, Status -Fragment | ForEach-Object {
    $_ -replace '<td>Fehler:', '<td class="error">Fehler:' -replace '<td>Erfolg</td>', '<td class="success">Erfolg</td>'
}

# Kombiniere beide Tabellen in einer einzigen HTML-Datei
$HtmlBody = @"
<h1>Tabelle der PCs mit Administratoren</h1>
$HtmlTableAdmins

<h1>Gemischte Liste aller PCs</h1>
$HtmlTableAll
"@

ConvertTo-Html -Head $HtmlHead -Body $HtmlBody | Out-File -FilePath $HtmlPath -Encoding utf8
